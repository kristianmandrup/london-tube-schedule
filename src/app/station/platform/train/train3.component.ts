...
export class TrainComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: import("@angular/core").SimpleChanges): void {
    const { train } = changes;
    const now = new Date();
    const expected = new Date(train.currentValue.expectedArrival);
    const gap = expected.getMinutes() - now.getMinutes();
    this.setByGap(gap);

    this.towards = train.currentValue.towards;
    this.orderColors = OrderColors[train.currentValue.lineId];
  }
