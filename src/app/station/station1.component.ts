...
export class StationComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.stationService.arrivals$.subscribe(data => {
      ...
      this.arrivals.map((arrival: Arrival) => {
        if (!this.platformMap[arrival.platformName]) {
          this.platformMap[arrival.platformName] = [];
        }
        this.platformMap[arrival.platformName].push(arrival);
      });
    });
  }
}
